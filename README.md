...
| 🪧 **[headwork](https://github.com/dbhq-uk/headwork-skill)**<br>`headwork@dbhq` | Puts the decision blocking you as a single question, explained in plain English first, with every option carrying its own justification and one of them named as the recommendation. Looks before it asks - the session, the git history, the open issues, the register - and states what it checked, so a question a file could have answered is a bug rather than a style lapse. Never goes hunting for work, never edits anything, and keeps no state |
