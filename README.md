...
| 📋 **[trello](https://github.com/dbhq-uk/trello-skill)**<br>`trello@dbhq` | Trello skill pack for Claude Code and Codex over the Trello REST API - boards, lists and cards end to end, a status digest for a board, and a radar for what is due across every board |
...
| 🪵 **[dovetail](https://github.com/dbhq-uk/dovetail-skill)**<br>`dovetail@dbhq` | Checks whether a repository agrees with itself - broken links, dangling anchors, orphaned files, duplicate content and stale translations from deterministic checks, and contradictions from model reviewers. By default only proven findings gate a pull request. The scan never writes; fixes are applied one at a time, and only the ones you approve |
...
| ⛵ **[vela](https://github.com/dbhq-uk/vela-skill)**<br>`vela@dbhq` | Compiler-exact code search over a SCIP index - where a symbol is defined, every reference, its direct callers and what implements it. .NET is indexed natively through Roslyn, with Razor and Blazor references resolved by the compiler into a saved index you query with no server running, and any other language reaches the same database through its own SCIP indexer. Nothing stays resident and it never modifies your repo. The plugin installs the skill; the vela command comes from the repository's installer |
