...
| 🌳 **[devskills](https://github.com/dbhq-uk/devskills)**<br>`devskills@dbhq` | A pack of development skills - gitview surveys every branch and worktree and says which branches are finished, test-merging to decide it rather than trusting a commit count that a squash-merge makes meaningless, then offers to clear them away; jira creates and reads Jira Cloud issues, checking the project key, issue type and required fields before it writes |
